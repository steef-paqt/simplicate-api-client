...
class PostSalesSale extends BaseEndpoint
{
     ...
     */
    public function __construct(\Steefdw\Simplicate\Model\PostSales $body)
    {
        $this->body = $body;
    }
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (401 === $status) {
            throw new \Steefdw\Simplicate\Exception\PostSalesSaleUnauthorizedException($response);
        }
        if (404 === $status) {
            throw new \Steefdw\Simplicate\Exception\PostSalesSaleNotFoundException($response);
        }
        if (500 === $status) {
            throw new \Steefdw\Simplicate\Exception\PostSalesSaleInternalServerErrorException($response);
        }
    }
