...
class GetSalesSalescustomfieldgroup extends BaseEndpoint implements Endpoint
{
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (401 === $status) {
            throw new \Steefdw\Simplicate\Exception\GetSalesSalescustomfieldgroupUnauthorizedException($response);
        }
        if (404 === $status) {
            throw new \Steefdw\Simplicate\Exception\GetSalesSalescustomfieldgroupNotFoundException($response);
        }
        if (500 === $status) {
            throw new \Steefdw\Simplicate\Exception\GetSalesSalescustomfieldgroupInternalServerErrorException($response);
        }
    }
