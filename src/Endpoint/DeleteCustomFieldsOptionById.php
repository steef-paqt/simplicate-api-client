...
class DeleteCustomFieldsOptionById extends BaseEndpoint
{
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (401 === $status) {
            throw new \Steefdw\Simplicate\Exception\DeleteCustomFieldsOptionByIdUnauthorizedException($response);
        }
        if (404 === $status) {
            throw new \Steefdw\Simplicate\Exception\DeleteCustomFieldsOptionByIdNotFoundException($response);
        }
        if (500 === $status) {
            throw new \Steefdw\Simplicate\Exception\DeleteCustomFieldsOptionByIdInternalServerErrorException($response);
        }
    }
